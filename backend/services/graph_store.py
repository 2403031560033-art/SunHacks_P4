...
class GraphStore:
    """MongoDB-based graph database scoped per user."""

    def __init__(self, user_id=None):
        self._user_id = user_id
        # Define default legacy user_id if none provided
        if not self._user_id:
            self._user_id = "legacy_shared"
        
        self.coll = get_db()['graphs']
        self.data = self._load()

    def _load(self):
        doc = self.coll.find_one({"user_id": self._user_id})
        if doc:
    ...
    def _save(self):
        self.coll.update_one(
            ...
            {"$set": {"nodes": self.data["nodes"], "edges": self.data["edges"]}},
            upsert=True
        )
    ...
    def clear(self):
        ...
        self.coll.delete_many({"user_id": self._user_id})
