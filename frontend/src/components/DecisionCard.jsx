    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-white leading-snug group-hover:text-accent-purple-light transition-colors">
            {decision.decision}
          </h3>
        </div>
        <div className={`flex items-center gap-1.5 ml-3 px-2.5 py-1 rounded-full text-[11px] font-medium ${confidenceColor} bg-dark-600/80`}>
      ...
      {decision.reasoning && decision.reasoning.length > 0 && (
        <div className="mb-3 space-y-1.5">
          <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Reasoning</p>
          {decision.reasoning.slice(0, 3).map((r, i) => (
            <div key={i} className="flex items-start gap-2">
              <ChevronRight className="w-3 h-3 text-accent-purple mt-0.5 shrink-0" />
      ...
      {decision.alternatives && decision.alternatives.length > 0 && (
        <div className="mb-3">
          <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Alternatives Considered</p>
          <div className="flex flex-wrap gap-1.5">
            {decision.alternatives.map((alt, i) => (
              <span key={i} className="px-2.5 py-1 rounded-lg text-[11px] bg-dark-600/80 text-gray-400 border border-white/5">
                {alt}
              </span>
            ))}
          </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-white/5">
        ...
        {decision.source_document && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-gray-600">{decision.source_document}</span>
          </div>
        )}
      </div>
